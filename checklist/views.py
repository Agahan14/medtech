...
class MedCardView(viewsets.ModelViewSet):
    ...
    queryset = MedCard.objects.filter(is_archive=False)
    lookup_field = 'patient'
    # permission_classes = (IsAuthenticatedOrReadOnly,
    #                       IsSuperUserOrOfficeManagerOrDoctor,)


class ArchiveMedCardView(MedCardView):
    queryset = MedCard.objects.filter(is_archive=True)


class CheckListView(viewsets.ModelViewSet):
    serializer_class = CheckListSerializer
    queryset = CheckList.objects.filter(is_archive=False)
...
class ArchiveCheckListView(CheckListView):
    queryset = CheckList.objects.filter(is_archive=True)
